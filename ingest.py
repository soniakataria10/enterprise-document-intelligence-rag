...
def document_hash_exists(document_hash: str) -> bool:
    ...
    result = vectorstore.get(where={"document_hash": document_hash}, limit=1)
...
def ingest_single_pdf(file_path, document_hash):
    ...
    for index, chunk in enumerate(chunks):
        chunk.metadata["chunk_id"] = f"{file_name}__chunk_{index}"
        chunk.metadata["chunk_index"] = index

    # Add ONLY new documents chunks
    create_store(chunks, embeddings)

    return {"status": "added", "file_name": file_name, "chunk_count": len(chunks)}
...
def delete_document(file_name):
    ...
    result = vectorstore.get(where={"document_id": file_name})
    ids = result.get("ids", [])
...
def delete_all_documents():
    ...
    return {"documents": len(deleted_files), "chunks": len(ids), "files": deleted_files}
